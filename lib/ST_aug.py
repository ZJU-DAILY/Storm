...
def build_kd_tree(adj):
    global kd_tree, node_positions, close_threshold, mid_threshold, distances_list, indices_list

    # 使用MDS将邻接矩阵（距离矩阵）转化为节点的二维坐标
    mds = MDS(n_components=2, dissimilarity='precomputed', random_state=0)
    # symmetrize_adj = adj
    symmetrize_adj = adj.cpu()
...
def time_noise(adj, x, noise_level=0.1):
    ...
    def add_noise_with_kdtree(time_series, adj, kd_tree, node_positions, close_threshold, mid_threshold,
        ...
        noise = torch.randn(bs, num_features, num_nodes, length) * noise_level
        noise = noise.to(x.device)
        # 使用KD树找到每个节点的所有邻居
        for i in range(num_nodes):
            distances, indices = distances_list[i], indices_list[i]
            ...
            if close_neighbors:
                ...
                time_series[:, :, i, :] += noise[:, :, j, :] * 0.5
            if mid_neighbors:
                j = np.random.choice(mid_neighbors)
                time_series[:, :, i, :] += noise[:, :, j, :] * 0.3
            if far_neighbors:
                j = np.random.choice(far_neighbors)
                time_series[:, :, i, :] += noise[:, :, j, :] * 0.2
