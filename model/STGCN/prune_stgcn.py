...
class align(nn.Module):
    def __init__(self, c_in, c_out):
        ...
        self.c_out = c_out
        if c_in > c_out:
            self.conv1x1 = mask_Conv2d(c_in, c_out, 1)
...
class temporal_conv_layer(nn.Module):
    def __init__(self, kt, c_in, c_out, act="relu"):
        ...
        if self.act == "GLU":
            self.conv = mask_Conv2d(c_in, c_out * 2, (kt, 1), 1)
        else:
            self.conv = mask_Conv2d(c_in, c_out, (kt, 1), 1)
...
class output_layer(nn.Module):
    def __init__(self, c, T, n, H):
        ...
        self.ln = mask_LayerNorm([n, c])
        self.tconv2 = temporal_conv_layer(1, c, c, "sigmoid")
        # self.fc = mask_Conv2d(c, 1, 1)
        self.fc = mask_Conv2d(1, H, kernel_size=(1, c), bias=True)
