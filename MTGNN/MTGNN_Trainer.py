...
class Trainer(object):
    def __init__(self, args, train_loader, val_loader, test_loader, scaler, model, loss, optimizer, lr_scheduler=None, freeze=True):
    ...
    def mkd_train_epoch(self, teacher_model, alpha, model_lambda, adj_mx, stu):
        self.model.train()
        total_loss = 0
        for _, (data, target) in enumerate(self.train_loader):
            data = data[..., :self.args.input_dim]
            ...
            # aug_teacher_model_output = teacher_model(aug_x)
            #
            # aug_predict = self.scaler.inverse_transform(aug_output)
            # aug_teacher_predict = self.scaler.inverse_transform(aug_teacher_model_output)
            # loss_d1 += self.loss(aug_predict, aug_teacher_predict)
            # loss += (model_lambda / 2) * loss_d1
    ...
    def train(self):
        ...
        for epoch in tqdm(range(1, self.args.epochs + 1)):
            ...
            print('Epoch {:03d}, Train Loss: {:.4f}, Valid Loss: {:.4f}, Training Time: {:.4f} secs, Inference Time: {:.4f} secs.'.format(epoch, train_epoch_loss, val_epoch_loss, (t2 - t1), (t3 - t2)))
    ...
    def mkd_train(self, teacher_model, alpha, model_lambda, adj_mx, stu):
        ...
        for epoch in tqdm(range(1, self.args.epochs + 1)):
            ...
            t1 = time.time()
            train_epoch_loss = self.mkd_train_epoch(teacher_model, alpha, model_lambda, adj_mx, stu)
            t2 = time.time()
            # 验证, 如果是Encoder-Decoder结构，则需要将epoch作为参数传入
            val_epoch_loss = self.val_epoch()
            t3 = time.time()
